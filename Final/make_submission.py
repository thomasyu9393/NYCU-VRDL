...
SUM_FEATURES = ['sum', 'mean', 'max'] + ['sum-{:.2}'.format(th) for th in SUM_THRESHOLDS]  #, 'mean', 'max'
...
FEATURE_NAMES = ['sum', 'mean', 'max', 'sum-0.04', 'sum-0.08', 'sum-0.24']
    ...
    for cls in range(utils.N_CLASSES):
        ...
        for x0 in steps(max_x):
            for y0 in steps(max_y):
                ...
                if patch.size:
                    features.extend([patch.mean(), patch.max()])
                else:
